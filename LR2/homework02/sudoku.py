...
def group(values: tp.List[T], n: int) -> tp.List[tp.List[T]]:
    ...
    """
    length = len(values)
    return [ values[i*length // n: (i+1)*length // n] 
             for i in range(n) ]
...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    a = 3 * (row//3)
    b = 3 * (col//3)
    return [grid[a+row][b+col] for row in range(3) for col in range(3)]
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    res=[]
    for n in range(1,10):
        if str(n) not in check_block:
            if str(n) not in check_row:
                if str(n) not in check_col:
                    res.append(str(n))
    res=set(res)
    return res
...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    for i in range(1, 10):
        alist.append(str(i))
    for j in range(9):
        grid[0].append(rd.choice(alist))
        alist.remove(grid[0][j])
    for e in range(1, 9):
        grid.append([])
        for k in range(9):
            grid[e].append('.')
    ...
    if N > 81:
        ...
        while n != N:
            a, b = rd.randint(0, 8), rd.randint(0, 8)
            if grid[a][b] != '.':
                grid[a][b] = '.'
                n -= 1
        return grid
